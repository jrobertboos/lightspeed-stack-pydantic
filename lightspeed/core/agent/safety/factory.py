...
model. ``granite_guardian``'s ``config.model`` is resolved the same way, but is required --
:class:`~lightspeed.core.agent.safety.granite_guardian.capability.GraniteGuardian` has no
run-model fallback.
"""
...
from lightspeed.core.providers import ProviderRegistry
...
def _build_capability(config: SafetyCapabilityConfiguration) -> AgentCapability[Any]:
    ...
    if isinstance(config, GraniteGuardianSafetyConfiguration):
        model = ProviderRegistry().get_model(config.config.model)
