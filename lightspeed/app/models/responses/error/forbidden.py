...
from lightspeed.app.models.responses.constants import FORBIDDEN_DESCRIPTION
from lightspeed.app.models.responses.error.bases import AbstractErrorResponse

# Permission string used by model/provider override checks (parity with original Action).
_MODEL_OVERRIDE_PERMISSION = "model_override"
...
class ForbiddenResponse(AbstractErrorResponse):
    ...
    def model_override(cls) -> Self:
        ...
        return cls(
            response=(
                "This instance does not permit overriding model/provider in the "
                f"query request (missing permission: {_MODEL_OVERRIDE_PERMISSION}). "
                "Please remove the model and provider fields from your request."
            ),
            cause=(
                f"User lacks {_MODEL_OVERRIDE_PERMISSION} permission required "
                "to override model/provider."
            ),
        )
